...
st.set_page_config(
    ...
    page_icon="https://favicon.streamlit.app/~/+/media/.png",
    layout="wide",
...
with st.sidebar:
    ...
    for _, course in unique_courses.iterrows():
        ...
        checkbox_key = f"checkbox_{course['Ders Kodu']}"
        ...
        if checkbox_value:
            ...
            if not is_selected:
                ...
                selected_section_key = f"section_{course['Ders Kodu']}"
                ...
                add_course_to_schedule(selected_section_details, get_course_color(selected_section_details['Ders Kodu']))
            else:
                # Section seçimi değiştiyse güncelle
                sections = data[data['Ders Kodu'] == course['Ders Kodu']]
                section_options = sections.apply(
                    lambda x: f"{x['Ders Section']} - {x['Hoca']} - {x['Saat']}", axis=1)
                selected_section_key = f"section_{course['Ders Kodu']}"
                selected_section = st.selectbox(f"{course['Ders Kodu']} için section seçin:", section_options, key=selected_section_key)
                selected_section_details = sections[section_options == selected_section].iloc[0]
                # Mevcut seçili section'u kaldır
                remove_course_from_schedule(course)
                # Yeni seçili section'u ekle
                st.session_state['selected_courses'] = [
                    (selected_section_details, get_course_color(selected_section_details['Ders Kodu'])) if selected_course[0]['Ders Kodu'] == course['Ders Kodu'] and selected_course[0]['Ders Section'] == selected_section_details['Ders Section'] else selected_course
...
# Footer
st.markdown("---")
st.markdown("SBA7© 2024 - Tüm Hakları Saklıdır", unsafe_allow_html=True)
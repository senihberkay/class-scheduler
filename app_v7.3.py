...
st.markdown("""
    <style>
        /* Ders programını daha modern hale getir */
        .schedule-container {
            display: grid;
        ...
        /* "Ders Programını Sıfırla" butonunu kırmızı yap */
        .reset-button {
            background-color: #FF4B4B !important;
        }
...
def get_course_color(course_code):
    """Ders kodlarına rastgele renk atar."""
    if "course_colors" not in st.session_state:
        st.session_state["course_colors"] = {}
    if course_code not in st.session_state['course_colors']:
        # Her ders için rastgele bir renk atayın
        st.session_state['course_colors'][course_code] = "#{:06x}".format(random.randint(0, 0xFFFFFF))
    return st.session_state['course_colors'][course_code]
...
schedule_html = "<table border='1' width='100%'>"
# Gün başlıklarını ekleyelim
schedule_html += "<tr><th>Saat</th>"
for day in days:
    schedule_html += f"<th>{day}</th>"
schedule_html += "</tr>"

# Saatlere göre tabloyu oluştur
for hour in hours:
    schedule_html += f"<tr><td>{hour}</td>"
    for day in days:
        cell_value = schedule.at[hour, day] if not pd.isna(schedule.at[hour, day]) else "-"
        if cell_value != "-":
            # Çakışma kontrolü
            if "/" in cell_value:
                # Çakışan dersleri ayır ve alt alta listele
                courses = cell_value.split(" / ")
                courses_clean = [strip_html_tags(course) for course in courses]
                
                # Her kurs için renk ve HTML etiketini oluştur
                courses_display = ""
                for course in courses_clean:
                    course_code_match = re.search(r'\w+\s\d+', course)
                    if course_code_match:
                        course_code = course_code_match.group(0)
                        color = get_course_color(course_code)  # Her ders için rastgele renk
                    else:
                        color = "#cccccc"  # Varsayılan renk
                    
                    courses_display += f"<span style='background-color:{color}; padding:5px;'>{course}</span>"
                
                schedule_html += f"<td>{courses_display}</td>"
            else:
                # Çakışma yoksa rastgele renkli kutu
                course_code_match = re.search(r'\w+\s\d+', cell_value)
                if course_code_match:
                    course_code = course_code_match.group(0)
                    color = get_course_color(course_code)  # Her ders için rastgele renk
                else:
                ...
                clean_value = strip_html_tags(cell_value)  # HTML etiketlerini temizle
                schedule_html += f"<td><span style='background-color:{color}; padding:5px;'>{clean_value}</span></td>"
        else:
            schedule_html += "<td>-</td>"
    schedule_html += "</tr>"
...
####
# Ders Programını Sıfırla Butonu
# if st.button("🔄 Ders Programını Sıfırla", key="reset_button"):
#     st.session_state['selected_courses'] = []
#     st.rerun()

# PDF ve Excel İndirme Butonları (Daha Şık)
...
# st.write(st.session_state['course_colors'])  # Mevcut renkleri kontrol et

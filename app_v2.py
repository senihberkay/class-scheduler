...
with st.sidebar:
    ...
    for _, course in unique_courses.iterrows():
        ...
        if is_selected:
            ...
            if not any(course['Ders Kodu'] == selected_course[0]['Ders Kodu'] for selected_course in st.session_state['selected_courses']):
                ...
                selected_section = st.selectbox(f"{course['Ders Kodu']} için section seçin:", section_options, key=f"section_{course['Ders Kodu']}")
